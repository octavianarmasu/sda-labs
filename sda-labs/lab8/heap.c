...
static void __heap_insert_fix(heap_t *heap, int pos) {
  team_t *tmp_team;
  int p = GO_UP(pos);
  while (p >= 0 && heap->cmp(heap->arr[pos], heap->arr[p]) < 0) {
...
void heap_insert(heap_t *heap, team_t *team) {
  ...
  heap->arr[heap->size]->name =
      calloc(TEAM_NAME_LEN, sizeof(*heap->arr[heap->size]->name));
 ...
 */
team_t *heap_top(heap_t *heap) { return heap->arr[0]; }
...
int main(void) {
  ...
  while (!heap_empty(heap)) {
    tmp_team = heap_top(heap);
    printf("%s %d", tmp_team->name, tmp_team->score);
    heap_pop(heap);
    if (!heap_empty(heap)) printf("\n");
  }

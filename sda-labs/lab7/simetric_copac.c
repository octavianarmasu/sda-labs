...
/**
 * TODO
 * @brief Given the root of a binary tree, check if it is symmetric around its
 * center.
 */

int helper(b_node_t *l, b_node_t *r) {
  if (l == NULL && r == NULL) return 1;
  if (l == NULL || r == NULL) return 0;
  if (*(int *)l->data == *(int *)r->data) return 1;
  if (*(int *)l->data != *(int *)r->data) return 0;
  return helper(l->left, r->right) && helper(l->right, r->left);
}

int is_symmetric(b_node_t *root) {
  if (root == NULL) return 1;
  // verificam daca e complet
